...
#选择文档函数
    ...
    def choose_docx_func(self):
        self.open_file_name = QFileDialog.getOpenFileName(self, '选择文件', '.', "Word files(*.docx)")
...
#~~~~~~~~~~~~~~~~~~~~显示函数~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    def text_display(self, texttmp):
        ...
        if texttmp.find('open failed:') != -1:
            QMessageBox.warning(self, '打开文件失败', '打开' + texttmp[12:] + '失败' + '请确认文档是否打开或者模板文件存在且后缀名为docx！')
            self.tabWidget.setEnabled(True)
            return
        if texttmp == 'nofile':
            QMessageBox.information(self, '错误', '还没有选择文件（夹），点击"文件"菜单或者工具栏进行选择！')
            self.tabWidget.setEnabled(True)
            return
...
class create_dagang_zhuisu(QtCore.QThread):
    ...
    def run(self):
        ...
        for i in range(csx_tb_count):
            ...
            #准备填入的data
            data = {'xuqiu': [], 'dg_zhangjie': '', 'mingcheng': '', 'biaoshi': ''}
            data2 = {'xuqiu': [], 'dg_zhangjie': '', 'mingcheng': '', 'biaoshi': ''}
            if dagangfile.Tables[i].Rows.Count > 2:
                #注意win32com的Cell从1开始不是从0开始
                if dagangfile.Tables[i].Cell(1, 1).Range.Text.find('测试项名称') != -1:
                    #一个用例不变内容获取
...
class create_zidong(QtCore.QThread):
    ...
    def run(self):
        ...
        if (tmp_fill.strip().isdigit()) and (int(tmp_fill.strip()) < len(t_s_file.tables)):
...
class get_content(QtCore.QThread):
    ...
    def run(self):
        ...
        line_list = [self.parent.lineEdit_12.text(), self.parent.lineEdit_13.text(), self.parent.lineEdit_14.text()]
                ...
                for ce in r1.cells:
                    if line_list[0] != '' and ce.text == line_list[0]:
                        while stb.cell(row, col).text == line_list[0]:
                            col += 1
                        c_file.tables[0].cell(rownum, 0).text = stb.cell(row, col).text
                        break
                    col += 1
                col = 0
                for ce in r1.cells:
                    if line_list[1] != '' and ce.text == line_list[1]:
                        while stb.cell(row, col).text == line_list[1]:
                            col += 1
                        c_file.tables[0].cell(rownum, 1).text = stb.cell(row, col).text
                        break
                    col += 1
                col = 0
                for ce in r1.cells:
                    if line_list[2] != '' and ce.text == line_list[2]:
                        while stb.cell(row, col).text == line_list[2]:
                            col += 1
                        c_file.tables[0].cell(rownum, 2).text = stb.cell(row, col).text
                        break
                    col += 1
                row += 1
        try:
